...
function getAllProducts($filter=array(),$sort="")
{
	...
	if(count($filter)>0)
	{
		...
		/*if(isset($product_color_tbl) && $product_color_tbl!='')
		{
			$q.=" order by product_mrp asc ";
			left join product_color as t2
		}*/
		if(isset($store_id) && $store_id!="")
		{
			$q.=" and product_id IN (SELECT mapping_product_id FROM `product_mapping` WHERE mapping_store_id=$store_id)";
		}
		if(isset($supercategory_id) && $supercategory_id!="")
		{
			$q.=" and product_category in (select subcategory_id from subcategory where subcategory_category_id in (select category_id from categories where category_supercategory_id=$supercategory_id))";
		}
		if(isset($category_id) && $category_id!="")
		{
			$q.=" and product_category in (select subcategory_id from subcategory where subcategory_category_id=$category_id)";
		}
		if(isset($subcategory_id) && $subcategory_id!="")
		{
			$q.=" and product_category=$subcategory_id";
		...
		{
			$q.=" and product_name like '%$product_name%'";
		}
		...
		if(isset($search))
		{
			...
			foreach( $search_exploded as $search_each ) 
				{ 
					...
					if( $x == 1 ) 
						$construct .="product_name LIKE '%$search_each%' or product_code LIKE '%$search_each%'"; 
					else 
						$construct .=" AND product_name LIKE '%$search_each%' or product_code LIKE '%$search_each%' and product_highlights LIKE '%$search_each%'"; 
				} 
	...
	{
		$q.=" order by product_mrp desc ";
	}
	else
		$q.=" order by `product_priority` DESC, `product_id` DESC, `product_last_date` ASC";
...
function getProductMapping($filter=array())
{
	global $con;
...
function isMemberFavrite($member_id, $product_id)
{
	global $con;
