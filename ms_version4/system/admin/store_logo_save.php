<?php
	...
	if($_POST["submit"]=="Update Image")
	{
		...
		if($type=="image/jpeg" || $type=="image/jpg" || $type=="image/pjpeg" || $type=="image/gif" || $type=="image/x-png" || $type=="image/bmp" ||  $type=="image/png")
		{
			...
			$target_dir = "../images/store/storelogo/";
	...
	/*else if($_POST["submit"]=="Remove Image")
	{
		...
		unlink("../images/products/".$product["store_logo"]);
		$q = "update products set store_logo='' where product_id=$product_id";
		$res = mysql_query($q);
		header("location:store_logo.php?update=true&product_id=$product_id");
	}*/
	else
